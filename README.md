...
- `Hyper + ↵`[^2]: If the entry has an associated PDF file, open it with the default PDF reader. The filename must not contain `@`, and the citekey must not contains an underscore (`_`).
